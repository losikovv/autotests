...
public interface CollectorCourierAutoCheck extends Check, CollectorCourierAutoElement {

    @Step("Проверяем, что отображается картинка вакансии 'Cборщик-курьер на авто'")
    default void checkVacancyPictureVisible() {
        Kraken.waitAction().shouldBeVisible(vacancyCartImage);
    }

    @Step("Проверяем, что заголовок вакансии 'Cборщик-курьер на авто' соответствует ожидаемому: '{expectedText}'")
    ...
    @Step("Проверяем, что описание вакансии 'Cборщик-курьер на авто' соответствует ожидаемой: '{expectedText}'")
    ...
    @Step("Проверяем, что информация о доходе вакансии 'Cборщик-курьер на авто' соответствует ожидаемой: '{expectedText}'")
    ...
    @Step("Проверяем, что указанные преимущества вакансии 'Cборщик-курьер на авто' соответствует ожидаемым: '{expectedAdvantageList}'")
    ...
    @Step("Проверяем, что отображается кнопка 'Откликнуться' вакансии 'Cборщик-курьер на авто'")
    default void checkRespondButtonVisible() {
        Kraken.waitAction().shouldBeVisible(respond);
    }

    @Step("Проверяем, что отображается кнопка 'Подробнее' вакансии 'Cборщик-курьер на авто'")
    default void checkMoreInfoButtonVisible() {
        Kraken.waitAction().shouldBeVisible(moreInfo);
    }

    @Step("Проверяем, что кнопка 'Откликнуться' вакансии 'Cборщик-курьер на авто' заблокирована")
