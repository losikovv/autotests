...
public interface CollectorCourierCheck extends Check, CollectorCourierElement {
    ...
    @Step("Проверяем, что отображается картинка вакансии 'Cборщик-курьер'")
    default void checkVacancyPictureVisible() {
        Kraken.waitAction().shouldBeVisible(vacancyCartImage);
    }

    @Step("Проверяем, что заголовок вакансии 'Cборщик-курьер' соответствует ожидаемому: '{expectedText}'")
    ...
    @Step("Проверяем, что описание вакансии 'Cборщик-курьер' соответствует ожидаемой: '{expectedText}'")
    ...
    @Step("Проверяем, что информация о доходе вакансии 'Cборщик-курьер' соответствует ожидаемой: '{expectedText}'")
    ...
    @Step("Проверяем, что указанные преимущества вакансии 'Cборщик-курьер' соответствует ожидаемым: '{expectedAdvantageList}'")
    ...
    @Step("Проверяем, что отображается кнопка 'Откликнуться' вакансии 'Cборщик-курьер'")
    default void checkRespondButtonVisible() {
        Kraken.waitAction().shouldBeVisible(respond);
    }

    @Step("Проверяем, что отображается кнопка 'Подробнее' вакансии 'Cборщик-курьер'")
    default void checkMoreInfoButtonVisible() {
        Kraken.waitAction().shouldBeVisible(moreInfo);
    }

    @Step("Проверяем, что кнопка 'Откликнуться' вакансии 'Cборщик-курьер' заблокирована")
