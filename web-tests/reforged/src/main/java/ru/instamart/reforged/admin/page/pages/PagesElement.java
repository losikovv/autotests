...
public interface PagesElement {
    ...
    Link tableEntrySpecificEdit = new Link(ByKraken.xpathExpression("//a[contains(@href, '%s')]/following-sibling::a[@data-action='edit']"), "Редактирование конкретной строки таблицы статических страниц");
    Link tableEntrySpecificDelete = new Link(ByKraken.xpathExpression("//a[contains(@href, '%s')]/following-sibling::a[@data-action='remove']"), "Удаление конкретной строки таблицы статических страниц");
    Link tableEntryRow = new Link(ByKraken.xpathExpression("//a[@href='/%s']/ancestor::tr"), "Строка таблицы страниц");
