...
public final class BasicOrdersTests {
    ...
    public void successOrderWithFavProducts() {
        userData.set(UserManager.getQaUser());
        helper.addFavorites(userData.get(), DEFAULT_AUCHAN_SID, 1);
        helper.dropAndFillCartFromFavorites(userData.get(), DEFAULT_AUCHAN_SID);
    ...
    public void updateUserDataAfterCheckout() {
        ...
        userData.set(UserManager.getUser());
        helper.dropAndFillCart(userData.get(), DEFAULT_AUCHAN_SID, helper.getAddressBySid(DEFAULT_AUCHAN_SID));
        ...
        userEdit().checkFullName(userEdit().getName(), userData.get().getName());
    }
    ...
    public void successAddItemsInActiveOrder() {
        ...
        helper.makeOrderOnTomorrow(userData.get(), DEFAULT_AUCHAN_SID, 1);
        helper.setAddress(userData.get(), RestAddresses.Moscow.defaultAddress());
    ...
    public void successMultiOrder() {
        ...
        helper.dropAndFillCartMultiple(userData.get(), RestAddresses.Moscow.defaultAddress(), DEFAULT_METRO_MOSCOW_SID, DEFAULT_AUCHAN_SID);
    ...
    public void successCancelMultiOrderViaCancelOneOrder() {
        userData.set(UserManager.getQaUser());
        helper.makeMultipleOrder(userData.get(), RestAddresses.Moscow.defaultAddress(), DEFAULT_METRO_MOSCOW_SID, DEFAULT_AUCHAN_SID);
        shop().goToPage();
        shop().interactHeader().clickToLogin();
        shop().interactAuthModal().authViaPhone(userData.get());
        shop().interactHeader().checkProfileButtonVisible();
    ...
    public void successCompleteCheckoutWithNewPaymentCard3DSAlreadyIn() {
        ...
        helper.dropAndFillCart(userData.get(), DEFAULT_SID);
