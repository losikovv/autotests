...
import ru.instamart.api.request.v1.admin.OrderSidebarAdminV1Request;
import ru.instamart.api.response.v1.admin.OrderSidebarV1Response;
...
public final class OrderSidebarV1Test extends RestBase {
    ...
    public void getOrderDeliveryWindows() {
        ...
        Response response = OrderSidebarAdminV1Request.GET(order.getNumber());
        checkStatusCode200(response);
        Allure.step("Проверка ответа", () -> {
            checkResponseJsonSchema(response, OrderSidebarV1Response.class);
            Assert.assertEquals(response.as(OrderSidebarV1Response.class).getSidebar().getUser().getEmail(),
                    user.getEmail(),
                    "Email пользователя из заказа отличается от ожидаемого");
        });
    }
    ...
    public void getOrderDeliveryWindowsNotFound() {
        ...
        Response response = OrderSidebarAdminV1Request.GET(Generate.literalString(8));
