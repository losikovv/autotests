...
public final class OffersDao extends AbstractDao<Long, OffersEntity> {
    ...
    public OffersEntity getOfferByStoreId(Integer storeId) {
        final var offer = new OffersEntity();
        try (final var connect = ConnectionManager.getDataSource(Db.MYSQL_STF).getConnection();
             final var preparedStatement = connect.prepareStatement(String.format(SELECT_SQL, "*") + "WHERE store_id = ? AND published = 1 AND deleted_at IS NULL LIMIT 1")) {
    ...
    public OffersEntity getSkuByPricer(long retailerId, int storeId, long shippingCategoryId, String pricer) {
        ...
        try (final var connect = ConnectionManager.getDataSource(Db.MYSQL_STF).getConnection();
             final var preparedStatement = connect.prepareStatement(String.format(SELECT_SQL, "o.*") +
