...
import static ru.instamart.kraken.enums.OnDemandServices.SHADOW_CAT;

@Slf4j
public class ConnectionPgSQLShadowCatManager {
    ...
    private ConnectionPgSQLShadowCatManager() {
    }

    protected static void portForward() {
        K8sPortForward.getInstance().portForwardPgSQLService(SHADOW_CAT.getNamespace(), SHADOW_CAT.getPort());
    }
    ...
    protected static void initConnectionPool() {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            var proxyConnection = (Connection)
                    Proxy.newProxyInstance(ConnectionPgSQLShadowCatManager.class.getClassLoader(), new Class[]{Connection.class},
    ...
    protected static Connection open() {
        try {
            return DriverManager.getConnection(
                    String.format("jdbc:postgresql://localhost:%s/app", SHADOW_CAT.getPort()),
