...
public class ConfigDao extends AbstractDao<String, ConfigEntity> {
    ...
    public boolean addConfig(String configId, String formulaId, Integer step, Boolean isDisabled, Integer distance){
        try (Connection connect = ConnectionManager.getDataSource(Db.PG_SURGE_LEVEL).getConnection();
             PreparedStatement preparedStatement = connect.prepareStatement(INSERT_SQL + " (id, formula_id, step_surge_level, disabled, distance) " +
                     " VALUES (?::uuid, ?::uuid, ?, ?, ?) ")) {
            ...
            preparedStatement.setObject(5, distance);
