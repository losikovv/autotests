...
public class OrdersDao implements Dao<String, OrdersEntity> {
    ...
    public OrdersEntity findByOrderNumber(String orderNumber) {
        try (final var connect = ConnectionManager.getDataSource(Db.PG_ORDER).getConnection();
             final var preparedStatement = connect.prepareStatement(String.format(SELECT_SQL, "*") + " WHERE order_number = ? LIMIT 1")) {
