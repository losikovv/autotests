...
import static ru.instamart.kraken.enums.OnDemandServices.ORDER_SERVICE;
import static ru.instamart.kraken.config.EnvironmentProperties.DEFAULT_PGSQL_POOL_SIZE;

@Slf4j
public class ConnectionPgSQLOrderServiceManager {
    ...
    private ConnectionPgSQLOrderServiceManager() {
    }

    protected static void portForward() {
        K8sPortForward.getInstance().portForwardPgSQLService(ORDER_SERVICE.getNamespace(), ORDER_SERVICE.getPort());
    }
    ...
    protected static Connection open() {
        try {
            return DriverManager.getConnection(
                    String.format("jdbc:postgresql://localhost:%s/app", ORDER_SERVICE.getPort()),
