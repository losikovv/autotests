...
@Slf4j
public class ConnectionPgSQLEtaManager {
    ...
    private ConnectionPgSQLEtaManager() {
    }

    protected static void portForward() {
        K8sPortForward.getInstance().portForwardPgSQLService(ETA.getNamespace(), ETA.getPort());
    }
    ...
    protected static void initConnectionPool() {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            var proxyConnection = (Connection)
                    Proxy.newProxyInstance(ConnectionPgSQLEtaManager.class.getClassLoader(), new Class[]{Connection.class},
    ...
    protected static Connection open() {
        try {
            return DriverManager.getConnection(
                    String.format("jdbc:postgresql://localhost:%s/app", ETA.getPort()),
