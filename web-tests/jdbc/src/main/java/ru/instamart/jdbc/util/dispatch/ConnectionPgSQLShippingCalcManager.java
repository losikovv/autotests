...
import static ru.instamart.kraken.enums.OnDemandServices.SHIPPINGCALC;
import static ru.instamart.kraken.config.EnvironmentProperties.DEFAULT_PGSQL_POOL_SIZE;

@Slf4j
public class ConnectionPgSQLShippingCalcManager {
    ...
    private ConnectionPgSQLShippingCalcManager() {
    }

    protected static void portForward() {
        K8sPortForward.getInstance().portForwardPgSQLService(SHIPPINGCALC.getNamespace(), SHIPPINGCALC.getPort());
    }
    ...
    protected static void initConnectionPool() {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            var proxyConnection = (Connection)
                    Proxy.newProxyInstance(ConnectionPgSQLShippingCalcManager.class.getClassLoader(), new Class[]{Connection.class},
    ...
    protected static Connection open() {
        try {
            return DriverManager.getConnection(
                    String.format("jdbc:postgresql://localhost:%s/app", SHIPPINGCALC.getPort()),
