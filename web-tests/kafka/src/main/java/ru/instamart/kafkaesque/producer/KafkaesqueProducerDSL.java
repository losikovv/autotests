...
import ru.instamart.kafkaesque.producer.KafkaesqueProducer.DelegateCreationInfo;
import ru.instamart.kafkaesque.producer.KafkaesqueProducer.Record;
...
public class KafkaesqueProducerDSL<Key, Value> {
  ...
  private KafkaesqueProducerDSL(KafkaConf conf) {
    this.conf = conf;
  }

  public static <Key, Value> KafkaesqueProducerDSL<Key, Value> newInstance(KafkaConf conf) {
    return new KafkaesqueProducerDSL<>(conf);
  }
